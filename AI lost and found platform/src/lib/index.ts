...
export function buildEnhancedMatches(
  ...
  // Build TF-IDF corpus from all items
  const allDocs = [
  ...
  for (const lost of lostItems) {
    for (const found of foundItems) {
      ...
      // Hard gate: if category is completely different (Other vs Electronics),
      // never show as high confidence regardless of text score
